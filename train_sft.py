...
class TinkerSFTTrainer:
    ...
    def _prepare_dataloaders(self):
        ...
        if load_full:
            ...
            print("Loading 5 percent of dataset...")
            full_dataset = load_dataset(dataset_path, split="train[:43000]") #5% of dataset 
            ...
            try:
                for epoch in range(num_epochs):
                    ...
                    for step, batch in enumerate(train_loader):
                        ...
                        # The actual number of sequences in this specific batch
                        actual_batch_size = batch["input_ids"].size(0)
                        
                        # Accumulator for all unmasked tokens in the batch
                        ...
                        for i in range(actual_batch_size):
                            ...
                            # Count how many tokens are NOT padding (-100)
                            weights_tensor = (labels_tensor != -100).float()                        
                            total_active_tokens += weights_tensor.sum().item() 
                            
                            # Replace -100 with 0 for the API's target tokens
                        ...
                        # 1. Calculate the MEAN loss per token across the ENTIRE batch
                        if total_active_tokens > 0 and 'loss:sum' in fwdbwd_result.metrics:
                            loss = fwdbwd_result.metrics['loss:sum'] / total_active_tokens
                        else:
                            loss = sum(fwdbwd_result.metrics.values()) / actual_batch_size 
